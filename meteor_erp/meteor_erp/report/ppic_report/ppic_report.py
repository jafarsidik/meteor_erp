...
def get_data(filters):
    ...
    for d in data:
        ...
        # Ambil Nama Customer dari Sales Order (jika ada)
        d.customer = None
        if d.sales_order:
            d.customer = frappe.db.get_value("Sales Order", d.sales_order, "customer")

        # Hitung Balance to Produce
        d.balance_to_produce = flt(d.qty) - flt(d.produced_qty)
...
def get_columns(filters):
    columns = [
		...
		},
        {
	 	...
	 	{
			"label": _("Tanggal Rilis WO"),
			"fieldname": "creation",
			"fieldtype": "Date",
			"width": 150,
		},
		{
			"label": _("Nama Finished Goods"),
		...
		{"label": _("Qty WO"), "fieldname": "qty", "fieldtype": "Float", "width": 110},
		{"label": _("Qty Produced"), "fieldname": "produced_qty", "fieldtype": "Float", "width": 110},
		{
			"label": _("Balance to Produce"),
			"fieldname": "balance_to_produce",
			"fieldtype": "Float",
			"width": 130,
		},
    ]
    ...
    if not filters.get("status"):
        columns.append(
            {"label": _("Status WO"), "fieldname": "status", "width": 100},
        )
