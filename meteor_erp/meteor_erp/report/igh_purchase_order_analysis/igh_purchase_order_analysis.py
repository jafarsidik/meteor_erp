...
def get_data(filters):
    ...
    query = (
        ...
        .left_join(pr_item).on(pr_item.purchase_order_item == po_item.name)
        .left_join(pr).on(pr_item.parent == pr.name)
        .left_join(item).on(po_item.item_code == item.name)
        .left_join(batch).on(pr_item.batch_no == batch.name)
        .select(
            ...
            po.supplier,
            po_item.item_code,
            item.item_group.as_("item_type"),  # <-- Item Type
            pr_item.batch_no.as_("batch"),  # <-- Batch
            batch.expiry_date.as_("exp"),  # <-- Exp
            pr.posting_date.as_("tanggal_penerimaan"),  # <-- Tanggal Penerimaan
            pr.supplier_delivery_note.as_("no_surat_jalan"),  # <-- No Surat Jalan
            po_item.qty,
...
def get_columns(filters):
    ...
    columns.extend(
        [
            ...
            {"label": _("Exp"), "fieldname": "exp", "fieldtype": "Date", "width": 100},
            {"label": _("Tanggal Penerimaan"), "fieldname": "tanggal_penerimaan", "fieldtype": "Date", "width": 130},
            {"label": _("No Surat Jalan"), "fieldname": "no_surat_jalan", "fieldtype": "Data", "width": 130},
        ]
    )
    ...
    columns.extend(
        [
            ...
            {"label": _("Amount"), "fieldname": "amount", "fieldtype": "Currency", "width": 110, "options": "Company:company:default_currency", "convertible": "rate"},
            {"label": _("Billed Amount"), "fieldname": "billed_amount", "fieldtype": "Currency", "width": 110, "options": "Company:company:default_currency", "convertible": "rate"},
            {"label": _("Pending Amount"), "fieldname": "pending_amount", "fieldtype": "Currency", "width": 130, "options": "Company:company:default_currency", "convertible": "rate"},
            {"label": _("Received Qty Amount"), "fieldname": "received_qty_amount", "fieldtype": "Currency", "width": 130, "options": "Company:company:default_currency", "convertible": "rate"},
