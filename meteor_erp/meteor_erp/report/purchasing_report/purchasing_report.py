...
def get_data(filters):
	...
	query = (
		...
		# join ke Material Request
		.left_join(mr)
		.on(po_item.material_request == mr.name)
		.left_join(mr_item)
		...
		.select(
			...
			# --- tambahan field MR dan PO ---
			po.total_taxes_and_charges,
			mr.name.as_("material_request"),
			mr.status.as_("mr_status"),
			mr_item.item_code.as_("mr_item_code"),
			mr_item.schedule_date.as_("mr_required_date"),
			mr.transaction_date.as_("transaction_date_pr"),
