...
class RawMaterial(Document):
	...
	def after_insert(self):
		...
		doc = frappe.get_doc({
			...
			'custom_raw_material_group':self.item_group,
			...
			'is_customer_provided_item': self.is_customer_provided_item
		})
		doc.insert()
	...
	def on_update(self):
		...
		doc_s.custom_raw_material_group = self.item_group
		doc_s.item_group = "Raw Material"
		...
		doc_s.supplier_items = self.supplier_items
		doc_s.purchase_uom = self.purchase_uom
		doc_s.min_order_qty = self.min_order_qty
		doc_s.safety_stock = self.safety_stock
		doc_s.lead_time_days = self.lead_time_days
		doc_s.is_customer_provided_item = self.is_customer_provided_item
		doc_s.save()
