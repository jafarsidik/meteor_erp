...
class Product(Document):
	...
	def after_insert(self):
		...
		doc = frappe.get_doc({
			...
			'custom_product_group':self.product_type,
			...
			'brand':self.brand,
			'uoms':self.uoms,
			'create_new_batch': self.create_new_batch,
			'has_expiry_date': self.has_expiry_date,
			'retain_sample': self.retain_sample,
			'batch_number_series': self.batch_number_series,
		})
		doc.insert()
	...
	def on_update(self):
		...
		doc_s = frappe.get_doc('Item',self.name)
		doc_s.item_code = self.item_code
		doc_s.item_name = self.item_name
		doc_s.custom_product_group = self.product_type
		doc_s.item_group = "Products"
		doc_s.stock_uom = self.stock_uom
		doc_s.is_stock_item = self.is_stock_item
		doc_s.valuation_rate=self.valuation_rate
		doc_s.standard_rate=self.standard_rate
		doc_s.opening_stock=self.opening_stock
		doc_s.image=self.image
		doc_s.description = self.description
		doc_s.brand = self.brand
		doc_s.uoms = self.uoms
		doc_s.shelf_life_in_days = self.shelf_life_in_days
		doc_s.end_of_life = self.end_of_life
		doc_s.warranty_period = self.warranty_period
		doc_s.weight_per_unit = self.weight_per_unit
		doc_s.weight_uom = self.weight_uom
		doc_s.default_material_request_type = self.default_material_request_type
		doc_s.valuation_method = self.valuation_method
		doc_s.allow_negative_stock = self.allow_negative_stock
		doc_s.has_batch_no = self.has_batch_no
		doc_s.has_serial_no = self.has_serial_no
		doc_s.create_new_batch = self.create_new_batch
		doc_s.has_expiry_date = self.has_expiry_date
		doc_s.retain_sample = self.retain_sample
		doc_s.batch_number_series = self.batch_number_series
		doc_s.save()
